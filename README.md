...
接下来进入正文，我们会根据下面的几个方面来思考RxJava如何与Retrofit结合时需要考虑的体位
...
至于放回的数据格式，大家自己访问下链接就看到了，太长就不放进来了。
...
另外，我的ProgressDialog的show方法应该在哪调用呢？看样子只能在getMovie()这个方法里面调用了，换个地方发出请求就要在对应的Listener里面写。
...
接下来我们把创建**Retrofit**的过程封装一下，然后希望Activity创建Subscriber对象传进来。
...
```
其中subscriber是MainActivity的成员变量。至于为什么这么写，后面我们在来详谈。
...
按照我们之前的用法，使用Gson作为转行需要我们在创建subscriber对象是指定返回值类型，如果我们对不同的返回值进行封装的话，那可能就要有上百个Entity了，看着明明是很清晰的结构，却因为data的不确定性无奈了起来。
...
```
这样一来HttpResult就相当于一个包装类，将结果包装了起来。
...
既然我们有了相同的返回格式，那么我们可能就需要在获得数据之后进行一个统一的预处理，比如我们之后在<code>resultCode==0</code>时才表示请求成功，继续下一步操作，否则算做Api异常。
使用Rxjava的flatMap可以完成这一功能。

我们来修改HttpMethods下面的getTopMovie代码
```java
public void getTopMovie(Subscriber<MovieEntity> subscriber, int start, int count){
  movieService.getTopMovie(start, count)
      .flatMap(new Func1<HttpResult<List<Subject>>, Observable<List<Subject>>>() {
        ...
        public Observable<List<Subject>> call(HttpResult<List<Subject>> httpResult) {
            if (httpResult.getResultCode() != 0) {
                return Observable.error(new ApiException(httpResult.getResultCode()));
            }
...
有点乱了是吧，没关系我们一步一步的分解。
如果你对flatMap的功能不是很了解，请先看[给 Android 开发者的 RxJava 详解](http://gank.io/post/560e15be2dca930e00da1083)这篇文章。
我们在中间调用了flatMap方法，而flatMap需要一个Func1对象作为参数，那么我们就创建一个Func1对象。

而Func1中的call方法最终会返回一个Observable对象，这个对象是事件最终也会传递给subscriber的。
我们先简单粗暴的根据错误码，生产不同的Observable对象。如果有错误，我们统一将服务器的错误码封装起来，集中处理。如果没有错误，那么就讲Data中的数据向下传递，毕竟resultCode只是给我们做判断用的，对与Activity和Fragment来说是无关紧要的，Activity和Fragment应该只关心Data中的内容。

这么写实际上已经可以进行预处理了，但是有一个问题，还是泛型的问题，由于每次Data会有不同的类型，那么创建Func1对象的时候也会有不同的泛型，这样就Func1中的call方法明明可以是相同的代码却又不可避免的每个请求都写一遍。

解决这个问题的关键是call方法的返回值是一个Observable对象。我们不能封装创建Func1对象的过程，但是我们能封装起来创建Observable的过程。

我们给HttpMethods类添加一个静态方法flatResult
```java
    ...
    return Observable.create(new Observable.OnSubscribe<T>() {
        ...
        public void call(Subscriber<? super T> subscriber) {
            if (result.getResultCode() != 0) {
                subscriber.onError(new ApiException(result.getResultCode()));
            } else{
...
然后我用了一个Handler来封装了ProgressDialog。(估计这地方该有人来拍砖了吧，希望有更好的方法能够提出来~)
```java
